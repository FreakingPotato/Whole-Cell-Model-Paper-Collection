...
def scan_pdf_profiles() -> list[dict]:
    profiles = []
    for pdf_path in sorted(PDF_DIR.glob("*.pdf")):
        try:
            profiles.append(extract_pdf_profile(pdf_path))
        except Exception:
            continue
    return profiles
...
def sync_rows_with_pdfs(
...
) -> tuple[list[dict[str, str]], dict[str, dict[str, str]], dict[str, dict[str, str]], list[dict]]:
    rows = [dict(row) for row in rows]
    classes = dict(classes)
    organisms = dict(organisms)
    profiles = scan_pdf_profiles()
    ...
    refreshed_profiles = scan_pdf_profiles()
    for row in rows:
        if row["pdf_file"]:
            continue
        ...
        for candidate in ordered:
            if download_pdf(candidate, target):
                try:
                    downloaded_profile = extract_pdf_profile(target)
                except Exception:
                    target.unlink(missing_ok=True)
                    continue
                if not profile_matches_paper(row, downloaded_profile):
                    quarantine_pdf(downloaded_profile)
                    continue
    ...
    return rows, classes, organisms, refreshed_profiles
...
def write_paper_notes(paper_meta: dict[str, dict]) -> None:
    ...
    method_md = textwrap.dedent(
        ...
        ## Mechanistic models
        Rule-based, kinetic, stochastic, physical, or simulation-first studies that represent cellular processes explicitly.

        ## Machine Learning Model
        Data-driven or AI-first work where predictive learning is the central modeling strategy.

        ## Hybrid architectures
        Workflows that combine mechanistic simulation with data integration, structural constraints, or AI-assisted components.
        """
    )
...
def write_summary(rows: list[dict[str, str]], classes: dict[str, dict[str, str]], G: nx.Graph) -> None:
    ...
    lines = [
        ...
        "- Mechanistic models: {}".format(class_counts["Mechanistic models"]),
        "- Machine Learning Model: {}".format(class_counts["Machine Learning Model"]),
        "- Hybrid architectures: {}".format(class_counts["Hybrid architectures"]),
        "",
        "Outputs:",
...
<style>
  * { box-sizing: border-box; }
  ...
  #legend { position: absolute; left: 16px; bottom: 16px; display: flex; gap: 8px; background: rgba(15, 23, 42, 0.82); padding: 10px 12px; border-radius: 14px; border: 1px solid rgba(148, 163, 184, 0.2); backdrop-filter: blur(10px); z-index: 7; }
...
function relatedNodeIds(nodeId) {
  ...
  direct.forEach(id => related.add(id));
  ...
  if (selectedNode?.file_type === 'paper') {
    GRAPH_DATA.nodes
      ...
      .forEach(node => related.add(node.id));
  }
  return related;
}
...
function buildTargets(layoutName) {
  ...
  const grouped = {};
  renderGuides(ORGANISM_GROUP_ORDER, ORGANISM_GROUP_LABELS);
  for (const node of GRAPH_DATA.nodes.filter(node => node.file_type === 'paper')) {
...
document.getElementById('search').addEventListener('input', event => {
  ...
  if (!query) return;
  const match = GRAPH_DATA.nodes.find(node => {
    const haystack = [
      node.label,
      node.display_label,
      node.journal,
...
function renderGuides(labels, displayMap = null) {
  const guideRoot = document.getElementById('layout-guides');
  if (!labels || !labels.length) {
    guideRoot.innerHTML = '';
    return;
  }
  guideRoot.innerHTML = labels.map((label, index) => {
    const percent = ((index + 0.5) / labels.length) * 100;
    const displayLabel = displayMap && displayMap[label] ? displayMap[label] : label;
    return `
      <div class="guide-line" style="left:${percent}%"></div>
      <div class="guide-label" style="left:${percent}%">${escapeHtml(displayLabel)}</div>
    `;
  }).join('');
}
...
def main() -> int:
    ...
    rows, classes, organisms, profiles = sync_rows_with_pdfs(rows, classes, organisms, inventory_urls)
