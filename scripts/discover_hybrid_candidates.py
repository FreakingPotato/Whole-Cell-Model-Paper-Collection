#!/usr/bin/env python3
"""Agent D1: Topic discoverer for hybrid ML-MM cross-domain candidate papers.

Discovers up to 10 high-impact, cross-domain candidate papers per claim in
``metadata/hybrid_model_evidence.json``. Cross-domain = physics, chemistry,
materials, climate, fluids, biology, ML methods (NOT WCM-only).
...
* ``metadata/hybrid_external_papers.json`` -- paper registry (EXT-NNN)
* ``metadata/hybrid_evidence_candidates.json`` -- candidates per claim
* ``metadata/hybrid_discovery_log.json`` -- per-claim diagnostics
...
EVIDENCE_FILE = META_DIR / "hybrid_model_evidence.json"  # READ ONLY
WCM_PAPERS_FILE = META_DIR / "wcm_paper_metadata.json"  # READ ONLY
ALLOWLIST_FILE = META_DIR / "hybrid_discovery_venue_allowlist.json"
REGISTRY_FILE = META_DIR / "hybrid_external_papers.json"
CANDIDATES_FILE = META_DIR / "hybrid_evidence_candidates.json"
LOG_FILE = META_DIR / "hybrid_discovery_log.json"
...
CLAIM_QUERIES: Dict[str, Dict[str, List[str]]] = {
    ...
    "parallel-matched-predictions": {
        "openalex": [
            ...
            '"hybrid model" benchmark perturbation prediction',
    ...
    "parallel-agreement-disagreement": {
        "openalex": [
            '"hybrid model" "uncertainty quantification"',
...
def discover_for_claim(
    ...
    # dedup against WCM corpus + within-pool
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for claim_id in target_claims:
        ...
        for rank, rec in enumerate(kept, start=1):
            ...
            candidates_list.append({
                "id": f"candidate-{claim_id}-EXT{num:03d}",
                "paper_id": ext_id,
                "claim_ref": claim_id,
    ...
    print(f"Total unique EXT papers: {len(registry_sorted)}")
