...
multi-claim cap), and:
  ...
  1. Appends new EXT evidence points to the right claim in
     ``metadata/hybrid_model_evidence.json`` (existing rows untouched).
  2. Overwrites ``metadata/hybrid_evidence_candidates.json`` with the
     unified candidates record (every (paper, claim) row carries
     ``tier`` and ``promotion_status``).
  3. Writes a human-readable trace to
     ``metadata/hybrid_evidence_promotion_log.md``.
...
EVIDENCE_FILE = META / "hybrid_model_evidence.json"
...
def _format_evidence_id(claim_id: str, idx: int) -> str:
    """Produce a human-readable evidence id for an EXT row.

    Example: ``hybrid-embedded-closure-EXT003`` (3rd appended EXT row to
    the embedded-closure claim). The numeric portion picks up at the
    next free integer past existing EXT rows, but for the schema we
    simply use a stable index ≥ 100 so we never collide with the
    existing ``hybrid-<claim>-001`` style WCM evidence ids.
    """
    return f"hybrid-{claim_id}-EXT{idx:03d}"
...
def write_evidence_file(
    ...
    """Append EXT evidence points and write the evidence file. Returns
...
def write_log(
    ...
    lines.append(
        "- All EXT evidence points are appended with `screenshot_status: \"no_pdf\"`; "
        "Agent P will fetch PDFs and Agent H pass 2 will rewrite to "
        "`pending` / `ok` once screenshots are produced."
    )
    lines.append(
        "- All EXT rows carry `external_reference: true` so the validator's "
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"OK — promoted {promoted_count} EXT evidence points "
